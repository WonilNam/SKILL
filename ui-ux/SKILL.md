---
name: ui-ux
description: 设计、实现或审查产品界面与前端交互，包括页面/组件、响应式布局、状态反馈、设计系统、可访问性、动效及 Canvas/WebGL。当用户要求新建或重构 UI、修复布局/交互、实现设计稿或做视觉质量验收时使用；不用于无界面行为的普通前端逻辑修改。
---
...
如果任务是实现现成设计稿或截图，以忠实还原为主，不擅自重做视觉方向；如果没有设计输入，先从现有产品模式推导，再为真正影响产品方向的选择请求确认。
...
仅在设计方案或审查需要交付结构化说明时使用；直接实现任务优先交付可运行界面和验证结果：
...
- 至少检查键盘焦点、颜色对比、reduced motion、触控尺寸和横向溢出；无法自动验证的项目要明确列为手工检查。
- 浏览器 QA 默认使用本地、mock 或 staging 数据。删除、支付、发送、账号变更及其他真实外部提交只检查界面状态；执行真实动作需要用户明确授权。
